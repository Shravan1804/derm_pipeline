...
class PatchExtractor:
    """
    Extract patches from images either by random sampling or splitting the image as a grid.
    ...
    def __init__(self, patch_sizes, seed=42, dynamic_res=False, root_dir=None):
        ...
        """
        self.patch_sizes = (
            sorted(patch_sizes) if type(patch_sizes) == list else [patch_sizes]
        )
        np.random.seed(seed)
        self.base_resolution = (
            self.compute_base_resolution(root_dir) if dynamic_res else None
        )

    def compute_base_resolution(self, rdir):
        """
        Calculate median resolution from dir.
    ...
    def adjust_ps(self, ps, im_res):
        ...
        return (
            ...
            else int(pow(im_res * ps * ps / self.base_resolution, 0.5))
        )

    def dir_images_to_patches(self, dirname):
        """
        Convert images in directory to patches.
        ...
        return {
            os.path.basename(f): self.patch_grid(f) for f in files if f not in exclude
        }

    def sample_patches(self, img_path, nb, im_arr=None):
        """
        Randomly sample patches from image.
        ...
        im_arr = (
            self.load_image(img_path) if im_arr is None else self.maybe_resize(im_arr)
        )
        ...
        for ps in self.patch_sizes:
            ...
            idx_h, idx_w = (
                np.random.randint(low=0, high=1 + im_h - ps, size=nb),
                np.random.randint(low=0, high=1 + im_w - ps, size=nb),
            )
            patches.extend(
                [
                    PatchExtractor.create_pm(img_path, ps, 0, 0, h, w, True)
                    for h, w in zip(idx_h, idx_w)
                ]
            )
        return patches
    ...
    def patch_grid(self, img_path, im_arr=None):
        """
        Convert image into a grid of patches.
        ...
        for ps in self.patch_sizes:
            ps = self.adjust_ps(ps, im_h * im_w)
            oh, ow = PatchExtractor.compute_side_overlap(
                im_h, ps
            ), PatchExtractor.compute_side_overlap(im_w, ps)
            ...
            grid_idx = [
                PatchExtractor.create_pm(img_path, ps, oh, ow, a, b)
                for a in grid_h
                for b in grid_w
            ]
            ...
            patches.extend(
                grid_idx
                if grid_idx
                else [PatchExtractor.create_pm(img_path, ps, 0, 0, 0, 0)]
            )
        return patches
    ...
    def maybe_resize(self, im_arr):
        ...
        if smallest < ps:
            ...
            return cv2.resize(
                im_arr, (max(ps, int(w * ratio)), max(ps, int(h * ratio)))
            )
        else:
            return im_arr
    ...
    def load_image(self, img_path):
        ...
        """
        if common.is_path(img_path) and type(img_path) is not str:
    ...
    def save_patches(self, source, dest, dirname, grids):
        """
        Extract and writes patches to disk.
        ...
        for img, patches in grids.items():
            ...
            for pm in patches:
                cimg.save_img(
                    self.extract_patch(im, pm), os.path.join(dest, pm["patch_path"])
                )

    @staticmethod
    def extract_patch(im, pm):
        """
        Extract patch from image using patch map info.
    ...
    def are_neighbors(p1, p2, d=1):
        """
        Determine if two patches are neighbors with certain patch-wise distance.
        ...
        :return: bool
        """
        pm1, pm2 = PatchExtractor.patch_name_to_pm(p1), PatchExtractor.patch_name_to_pm(
            p2
        )
        ...
        (h1, w1), (h2, w2) = PatchExtractor.get_position(
            p1
        ), PatchExtractor.get_position(p2)
    ...
    def get_neighbors_dict(patches, d=1):
        ...
        """
        pidx = {
            p: i for i, p in enumerate(patches)
        }  # patch name to patch index in lst patches
        ...
        neigh_pidx = (
            []
        )  # item 0 corresponds to patch at index 0, item is a lst of neighboring patch indexes
        groups_pidx = [
            []
        ]  # item 0 corresponds to patches with 0 neighbors, item is lst of patch indexes
        pidx_groups = np.zeros(
            patches.size, dtype=np.int
        )  # patch index to corresponding patch group
        for p1 in patches:
            idxs = []
            ...
            if len(idxs) > len(groups_pidx) - 1:
                groups_pidx.extend(
                    [[] for _ in range(len(idxs) - (len(groups_pidx) - 1))]
                )
        ...
        return (
            neigh,
            np.array(neigh_pidx),
            np.array([np.array(g) for g in groups_pidx]),
            pidx_groups,
        )
    ...
    def compute_side_overlap(n, ps):
        """
        Compute minimum overlap between patches maximizing the number of patches.

        Height/width overlap can differ
    ...
    def get_overlap(patch_name):
        """
        Extract patch size from its filename.
    ...
    def get_position(patch_name):
        """
        Extract patch coordinates from its filename.
    ...
    def create_pm(img_path, ps, oh, ow, h, w, randomly_sampled=False, patch_name=None):
        ...
        if patch_name is None:
            sep = (
                PatchExtractor.SEP + PatchExtractor.RAND
                if randomly_sampled
                else PatchExtractor.SEP
            )
    ...
    def patch_name_re():
        """
        Define REGEX used to extract patch information from patch filename.

        :return: compiled regex
        """
        sep = rf"(?P<sep>{PatchExtractor.SEP}(?:{PatchExtractor.RAND})?)"
        reg = re.compile(
            rf"^(?P<img>.+){sep}_ps(?P<ps>\d+)_oh(?P<oh>\d+)_ow(?P<ow>\d+)_h(?P<h>\d+)_w(?P<w>\d+)$"
        )
        return reg
    ...
    def can_extract_pm_from_patch_name(patch_name):
        """
        Check if patch filename contains all necessary patch information.
    ...
    def patch_name_to_pm(patch_name):
        """
        Convert a patch filename to a patch map.
        ...
        assert (
            results
        ), f"Cannot extract necessary information from patch name: {patch_name}"
        img_path = results.group("img") + ext
        ps, oh, ow, h, w = tuple(
            (int(results.group(k)) for k in ["ps", "oh", "ow", "h", "w"])
        )
        randomly_sampled = PatchExtractor.RAND in results.group("sep")
        return PatchExtractor.create_pm(
            img_path, ps, oh, ow, h, w, randomly_sampled, patch_name
        )
    ...
    def rebuild_im_from_patches(pms, patches, full_shape=None, interpol=None):
        ...
        if full_shape is None:
            full_shape = cv2.imread(pms[0]["full_img"], cv2.IMREAD_UNCHANGED).shape
    ...
    def impath_to_patches(img_path, ps):
        """
        Tile full image to patches.

        :param img_path: str, full image path
        :param ps: int, patch size
        :return: tuple with list of patches and list of patch maps
        """
        return PatchExtractor.im_to_patches(
            PatchExtractor([ps]).load_image(img_path), ps, img_path
        )

    @staticmethod
    def im_to_patches(im, ps, imname="fi_img.jpg"):
        """
        Convert full image array to patches, resizing the image if it was too small with respect to patch size.
        ...
        return (
            im_resized,
            [PatchExtractor.extract_patch(im_resized, pm) for pm in pms],
            pms,
        )
...
def multiprocess_patching(proc_id, pmq, patcher, data, dirs, dest):
    """
    Apply patching in multiprocess mode.
...
def get_dirs(root, level):
    """
    Get directories containing images at an arbitrary level in the file hierarchy.
    ...
    while level > 0:
        all_dirs = [
            os.path.join(a, b)
            for a in all_dirs
            for b in common.list_dirs(os.path.join(root, a))
        ]
        level -= 1
    return all_dirs
...
def main(args):
    """
    Perform the multiprocess patching of dataset according to command line args.
    ...
    patcher = PatchExtractor(
        args.patch_sizes, dynamic_res=args.dynamic_ps, root_dir=args.data
    )
    pmq = mp.Queue()
    jobs = []
    for i, dirs in zip(range(workers), batched_dirs):
        jobs.append(
            mp.Process(
                target=multiprocess_patching,
                args=(i, pmq, patcher, args.data, dirs, args.dest),
            )
        )
        jobs[i].start()
    ...
    pickle.dump(
        ...
        open(
            os.path.join(
                args.dest, f'patches_{"_".join(map(str, args.patch_sizes))}.p'
            ),
            "wb",
        ),
    )
...
def get_patcher_arg_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Creates patch dataset from image dataset"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--dest", type=str, help="directory where the patches should be saved"
    )
    parser.add_argument(
        "-p", "--patch-sizes", nargs="+", default=[512], type=int, help="patch sizes"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="nested level of class folders compared to args.data",
    )
    parser.add_argument(
        "--dynamic-ps",
        action="store_true",
        help="Adjusts ps for imgs res different from median res",
    )
    return parser
