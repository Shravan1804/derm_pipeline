...
def add_encrypted_args(parser):
    """
    Add crypto command line arguments to provided argument parser.
...
def encrypt_im(im, fkey, im_format="JPEG", save_path=None):
    """
    Encrypt image array, save it to disk if requested.
...
def decrypt_img(img_path, fkey):
    """
    Decrypt provided image file using Fernet key object and load it as np array.
...
def request_key(test_key_dir=None, user_key=None):
    """
    Request crypto key from user input and convert it to Fernet object key, testing it if requested.
    ...
    try:
        ...
        if test_key_dir is not None:
            crypted_key_files = [
                kf
                for kf in common.list_files(test_key_dir, full_path=True)
                if kf.endswith(".key")
            ]
            assert (
                len(crypted_key_files) > 0
            ), f"No crypted key in provided dir {test_key_dir}"
            crypted_key = crypted_key_files[0]
            with open(crypted_key, "rb") as kf:
                assert user_key == user_fkey.decrypt(
                    kf.read()
                ), f"Error with provided key, checked with {crypted_key}"
    except Exception:
...
def load_key(key_file):
    """
    Load key from provided file path.
...
def decrypt(encrypted_file_path, fkey):
    """
    Decrypt provided file using provided key.
...
def mp_encrypt(proc_id, files, fkey, root, dest, filenames_dict):
    """
    Multiprocess file encryption of provided files.
    ...
    for i, file in enumerate(files):
        ...
        encrypted_dest = file.replace(root, dest).replace(
            name + ext, filenames_dict[name] + ext
        )
...
def create_filenames_dict(files, save_path):
    """
    Create bidict mapping between encrypted filenames and original filenames.
...
def main(args):
    """
    Run the multiprocess encryption based on the provided command line arguments.

    :param args: command line args
    """
    fkey = load_key(args.key_file)
    files = common.list_files(args.data, full_path=True, recursion=True)
    filenames_dict = create_filenames_dict(
        files, os.path.join(args.data, f"{os.path.basename(args.data)}_filenames.p")
    )
    ...
    for i, files in zip(range(workers), batched_files):
        jobs.append(
            mp.Process(
                target=mp_encrypt,
                args=(i, files, fkey, args.data, args.dest, filenames_dict),
            )
        )
        jobs[i].start()
    for j in jobs:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="dataset to be encrypted"
    )
    parser.add_argument(
        "--dest",
        type=str,
        help="directory where the encrypted data is saved, default will be data"
        "dirname with _encrypted appended",
    )
    parser.add_argument(
        "--new-key", action="store_true", help="Creates new key file in --key-file"
    )
    parser.add_argument(
        "--key-file",
        type=str,
        help="filepath where to save key, default will be data dirname with"
    ...
    if args.new_key:
        if args.key_file is None:
            args.key_file = os.path.join(
                args.data, f"{os.path.basename(args.data)}.key"
            )
        ...
        assert (
            args.key_file is not None
        ), "Error either provide --key-file or --new-key parameters"
    ...
    if args.dest is None:
        args.dest = common.maybe_create(
            os.path.dirname(args.data), f"{os.path.basename(args.data)}_encrypted"
        )
    else:
