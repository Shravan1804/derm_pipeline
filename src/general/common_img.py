...
def quick_img_size(img_path):
    """
    Return height, width of img, quicker than cv2 since it does not load the image in memory.
...
def im_sharpness_score(im):
    """
    Compute a sharpness score of image.
...
def resize(im, new_h, new_w):
    """
    Resize function (h, w in correct order, cv2 uses w, h).
...
def load_img(path, resize=None):
    ...
    :return: array, image
    """
    if common.is_path(path) and type(path) is not str:
        path = str(path)
...
def h_w_ratio(impath, im=None):
    """
    Calculate image heigh/width ratio.
