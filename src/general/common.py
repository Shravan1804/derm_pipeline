...
def merge(lst, cond_fn, merge_fn):
    """
    Apply recursively merge fn on pairs of lst items if cond_fn is satisfied.
...
def decimal_to_base(n, base, ndigits=-1):
    """
    Convert n from base 10 to specified base.
    ...
    if ndigits != -1:
        assert (
            count <= ndigits
        ), f"{int(n)} cannot be converted in base {base} with only {ndigits} digits."
...
def generate_codes(ncodes, alphabet=ascii_uppercase, size=None):
    """
    Generate unique codes based on provided alphabet.
    ...
    if size != -1:
        error_msg = (
            f"Error, requested {ncodes} different codes but an alphabet with only {n_symbols}"
            f"different symbols was provided (require at least {min_symbols} different symbols)."
        )
        assert size >= min_symbols, error_msg
    return [
        "".join([alphabet[i] for i in decimal_to_base(c, len(alphabet), ndigits=size)])
...
def int_to_bins(n, bins, rand=False):
    """
    Perform integer division of number into bins.

    If rand true, random split else equal split
    ...
    if rand:
        temp = np.concatenate(
            [np.zeros(n, dtype=np.bool), np.ones(bins - 1, dtype=np.bool)]
        )
...
def equidistant_pts(mini, maxi, inter):
    """
    Prepare list of equidistant points between range separated by fixed distance.
    ...
    """
    return np.linspace(
        mini, maxi, np.int(np.round(np.abs(maxi - mini) / inter)) + 1, endpoint=True
    ).tolist()


def most_common(arr, top=3, return_index=False, return_counts=False):
    """
    Return most common elements in array.
...
def maybe_create(*d):
    """
    Receive arbitrary number of dirnames, joins them and create them if they don't exist.
...
def reproduce_dir_structure(source, dest):
    """
    Reproduce dir structure of source in dest.

    Raise exception if source invalid. Creates dest if needed.
...
def list_images(root, **kwargs):
    ...
    return [
        f
        for f in list_files(root, **kwargs)
        if os.path.splitext(f)[1] in (".jpg", ".png", ".tiff")
    ]


def list_files(
    root, full_path=False, posix_path=False, recursion=False, max_rec_level=-1
):
    """
    ...
    """
    lf = [
        os.path.join(root, f) if full_path else f
        ...
        for d in list_dirs(root, full_path, posix_path, recursion, max_rec_level - 1):
            ...
            lf.extend(
                [
                    ...
                    if os.path.isfile(os.path.join(full_d, f))
                ]
            )
...
def list_dirs(
    ...
    if recursion and rec_level > max_rec_level >= 0:
        return []
    ld = [
        os.path.join(root, d) if full_path else d
    ...
    if recursion:
        ...
        for d in ld:
            ...
            children = list_dirs(
                new_root, full_path, posix_path, recursion, max_rec_level, rec_level + 1
            )
...
def list_files_in_dirs(root, full_path=False, posix_path=False):
    """
    Return the list of files present in level 1 subdirs.

    Typically classification dataset format.
    :param root: str, dir to retrieve file from
    :param full_path: bool, whether to return full path
    :param posix_path: bool, whether to return paths as posix objs
    :return: list of files
    ...
    lf = [
        os.path.join(d, f)
        for d in list_dirs(root, full_path)
...
def check_file_valid(filepath):
    ...
    """
    assert os.path.exists(filepath) and os.path.isfile(
        filepath
    ), f"Provided file {filepath} invalid."
...
def check_dir_valid(dirpath):
    ...
    """
    assert os.path.exists(dirpath) and os.path.isdir(
        dirpath
    ), f"Provided dir {dirpath} invalid."
...
def elapsed_timer():
    """
    Return context manager timing whatever executes within.
    ...
    """
    start = default_timer()
    elapser = lambda: default_timer() - start  # noqa
    yield lambda: elapser()
    end = default_timer()
    elapser = lambda: end - start  # noqa


def time_method(m, *args, text="Work", **kwargs):
    """
    Use elapsed_timer context manager on function.
...
def stdout_prepend(f, pre_msg, *args):
    """
    Execute function f with stdout prepend.
