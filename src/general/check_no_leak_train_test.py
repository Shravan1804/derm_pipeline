...
def get_image_name(args, path):
    ...
    If dataset is patched or cropped, will look for full img name
...
def search_terms(proc_id, terms, search_in, args):
    """
    Search terms' items in search_in lst, prints number of matches, if verbose also prints the list of matches.
    ...
    if args.verbose:
        print(
            f"Proc {proc_id} searching for {len(terms)} terms in a list of {len(search_in)} items."
        )
    count = 0
    duplicates = {}
    for t, tt in terms:
        ...
        if args.verbose and count > 0 and count % 5000 == 0:
            print(
                f"Proc {proc_id} completed {count}/{len(terms)} lookups ({len(terms)-count} remaining)."
            )
        count += 1
    print(
        f"Proc {proc_id} found {len(duplicates.keys())} duplicates: {duplicates.keys()}"
    )
    if args.verbose:
...
def main(args):
    """
    Run the multiprocess search based on the provided command line arguments.

    :param args: command line args
    """
    terms, search_in = get_files_to_search(args)
    workers, batch_size, batched_dirs = concurrency.batch_lst(terms)
    concurrency.multi_process_fn(
        workers, batched_dirs, partial(search_terms, search_in=search_in, args=args)
    )
    print("done")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir1", type=str, required=True, help="first directory (e.g. train dir)"
    )
    parser.add_argument(
        "--dir2", type=str, required=True, help="second directory (e.g. test dir)"
    )
    parser.add_argument(
        "--classif", action="store_true", help="expect class subdirs in provided dirs"
    )
    parser.add_argument(
        "--segm", action="store_true", help="if dataset is segm dataset"
    )
    common_segm_args(parser)
    parser.add_argument(
        "--crops",
        action="store_true",
        help="dir contains imgs_crops => checks if same orig img",
    )
    parser.add_argument(
        "--patch",
        action="store_true",
        help="dir contains patches => checks if same img in both dirs",
    )
    parser.add_argument(
        "--patch-sep", type=str, default="__SEP__", help="patch name separator"
    )
