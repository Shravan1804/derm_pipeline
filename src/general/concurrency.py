...
def add_multi_proc_args(parser):
    """
    Add concurrency args: workers and bs.
...
def batch_lst(files, bs=None, workers=None):
    ...
    n = len(files)
    if workers is None:
        workers = min(mp.cpu_count() - 2, n)
    if bs is None or bs * workers < n:
        bs = math.ceil(n / workers)
    return workers, bs, common.batch_list(files, bs)


def unload_mpqueue(pmq, processes):
    """
    Empty multiprocess queue.
...
def multi_process_fn(workers, batches, fn):
    """
    Dispatch function to workers with batches as args.
