...
def create_segm_masks(args, proc_id, images, coco):
    """
    Create segmentation masks from coco object annotations.

    Also writes the mask to disk.
...
def get_images_without_labels(args, img_with_labels):
    """
    Retrieve images without object annotations.

    They should still be gathered to later create an empty mask.
    ...
    for f in common.list_files(
        os.path.join(os.path.dirname(args.json), args.img_dir), full_path=True
    ):
        filename = os.path.basename(f)
        if filename not in has_labels:
            h, w = cimg.quick_img_size(f)
            no_labels.append(
                SimpleNamespace(id=-1337, file_name=filename, height=h, width=w)
            )
    return no_labels
...
def main(args):
    """
    Perform the multiprocess coco to segmentation dataset conversion.
    ...
    np.random.shuffle(
        all_images
    )  # otherwise last process has all the images without annos
    workers, batch_size, batched_images = concurrency.batch_lst(
        all_images, workers=args.workers
    )
    jobs = []
    for proc_id, images in zip(range(workers), batched_images):
        jobs.append(
            mp.Process(target=create_segm_masks, args=(args, proc_id, images, coco))
        )
        jobs[proc_id].start()
    for proc_id in jobs:
        proc_id.join()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json", type=str, required=True, help="COCO json labels to be converted"
    )
    parser.add_argument(
