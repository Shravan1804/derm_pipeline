...
class ImageClassificationInference(ImageInference):
    """Class used to perform inference with classification model, based on classification trainer."""
    ...
    def get_argparser(parser):
        ...
        """
        parser = super(
            ImageClassificationInference, ImageClassificationInference
        ).get_argparser(parser)
        parser.add_argument(
            "--topk", type=int, default=2, help="Top probas to be written on img"
        )
        parser.add_argument(
            "--gradcam", action="store_true", help="Show gradcam of predictions"
        )
        return parser
    ...
    def prepare_inference(args):
        """
        Call classification trainer setup.
    ...
    def learner_preds(self, learn, dl):
        """
        Compute learner predictions on inference items. Redefined here to be able to perform gradcam.
            ...
            with GPUManager.running_context(learn, self.args.gpu_ids):
                for b in dl:
                    ...
                    if len(b) == 1:
                        ...
                        interp.targs.append(b[1])
                    with fv.Hook(
                        learn.model[0],
                        lambda m, i, o: o[0].detach().clone(),
                        is_forward=False,
                    ) as hookg:
                        with fv.Hook(
                            learn.model[0], lambda m, i, o: o.detach().clone()
                        ) as hook:
                            output = learn.model.eval()(
                        ...
                        grad = hookg.stored
                    interp.gradcam.append(
                        (grad[0].mean(dim=[1, 2], keepdim=True) * act[0]).sum(0).cpu()
                    )
            return interp
        else:
            return super().learner_preds(learn, dl)
    ...
    def process_results(self, inference_item, interp, save_dir):
        ...
        if interp.pms is None:
            hmap = cv2.resize(hmap, im.shape[:2], interpolation=cv2.INTER_LINEAR)
        else:
            hmap = PatchExtractor.rebuild_im_from_patches(
                interp.pms, hmap, im.shape[:2], interpol=cv2.INTER_LINEAR
            )
        interp.gradcam = hmap
    ...
    def plot_results(self, interp, im, gt, pred, save_path):
        """
        Plot inference results: original image with predicted labels, eventually heatmap, eventually metrics.
        ...
        pred_pos = (
            [(0, 0)]
            if interp.pms is None
            else [(pm["h"], pm["w"]) for pm in interp.pms]
        )
...
def main(args):
    """
    Perform classification inference based on provided args.
...
if __name__ == "__main__":
    ...
    parser = ImageClassificationInference.get_argparser(
        ImageClassificationTrainer.get_argparser(pdef=pdef)
    )
