...
from ..classification import classification_utils as classif_utils
...
class ImageClassificationTrainer(ImageTrainer):
    ...
    @staticmethod
    def get_argparser(
        desc="Fastai image classification trainer arguments", pdef=dict(), phelp=dict()
    ):
        """
        Create classification argparser.
        ...
        """
        psr = super(
            ImageClassificationTrainer, ImageClassificationTrainer
        ).get_argparser(desc, pdef, phelp)
        psr.add_argument(
            "--oversample",
            action="store_true",
            help="Uses weighted dls based on class distrib",
        )
        return psr
    ...
    def get_exp_logdir(args, custom=""):
        ...
        return super(
            ImageClassificationTrainer, ImageClassificationTrainer
        ).get_exp_logdir(args, custom=custom)
    ...
    def prepare_training(args):
        ...
        if args.cats is None:
            args.cats = common.list_dirs(
                os.path.join(args.data, args.sl_train[0]), full_path=False
            )
        if args.exp_logdir is None:
            args.exp_logdir = os.path.join(
                args.logdir, ImageClassificationTrainer.get_exp_logdir(args)
            )
        super(ImageClassificationTrainer, ImageClassificationTrainer).prepare_training(
            args
        )
    ...
    def create_cats_metrics(self, perf_fn, cat_id, cat, metrics_fn):
        """
        Generate metrics functions for the individual categories.
    ...
    def ordered_test_perfs_per_cats(self):
        ...
        return [
            ([self.get_cat_metric_name(f, c) for c in all_cats], f)
            for f in self.args.metrics_base_fns
        ]
    ...
    def compute_conf_mat(self, targs, decoded):
        ...
        return classif_utils.conf_mat(
            fv.TensorBase(targs), fv.TensorBase(decoded), self.args.cats
        )
    ...
    def create_dls(self, tr, val, bs, size):
        ...
        if self.args.oversample:
            kwargs = {
                "dl_type": fv.WeightedDL,
                "wgts": self.get_train_items_weights(list(zip(*tr))),
    ...
    def get_train_items_weights(self, train_items):
        """
        Compute each train item weight based on class weights.

        :param train_items: tuple of fastai lists, (items, labels)
        :return:
        """
        labels, class_weights = [x[1] for x in train_items], self.get_class_weights(
            train_items
        ).numpy()
    ...
    def create_learner(self, dls):
        ...
        if "efficientnet" in self.args.model:
            ...
            msplitter = lambda m: fv.L(train_utils.split_model(m, [m._fc])).map(  # noqa
                fv.params
            )
            learn = fv.Learner(
                dls, model, splitter=msplitter, cbs=callbacks, **learn_kwargs
            )
    ...
    def precompute_metrics(self, interp):
        ...
        return {
            cid: metrics.get_cls_TP_TN_FP_FN(t == cid, d == cid)
...
if __name__ == "__main__":
    ...
    parser = ImageClassificationTrainer.get_argparser(
        desc="Fastai image classification", pdef=defaults
    )
