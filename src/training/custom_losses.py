...
class GeneralizedDiceLoss(fv.Module):
    ...
    def __init__(self, axis=1, eps=1e-7):
        fv.store_attr()
    ...
    def forward(self, out, targs):
        ...
        o = torch.softmax(out, dim=self.axis).clip(self.eps, 1.0 - self.eps)
        ...
        loss = 1 - 2 * (torch.einsum("bk->b", inter) + self.eps) / (
            torch.einsum("bk->b", union) + self.eps
        )
        return loss.mean()
...
class DiceLoss(fv.Module):
    ...
    def __init__(self, axis=1, delta=0.5, eps=1e-7, smooth=1e-6):
        fv.store_attr()
        self.coeff = 0
...
class FocalDiceLoss(DiceLoss):
    def __init__(self, axis=1, delta=0.5, eps=1e-7, smooth=1e-6, gamma=0.75):
...
class TverskyLoss(DiceLoss):
    def __init__(self, axis=1, delta=0.7, eps=1e-7, smooth=1e-6):
        super().__init__(axis, delta, eps, smooth)


class FocalTverskyLoss(FocalDiceLoss):
    def __init__(self, axis=1, delta=0.7, eps=1e-7, smooth=1e-6, gamma=0.75):
        super().__init__(axis, delta, eps, smooth, gamma)
