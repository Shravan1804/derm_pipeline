...
from ..training import custom_losses as closs
...
class ImageTrainer(FastaiTrainer):
    ...
    def get_argparser(desc="Fastai image trainer arguments", pdef=dict(), phelp=dict()):
        """Create image trainer argparser.
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            default=pdef.get("--cats", None),
            help=phelp.get("--cats", "Object categories"),
        )

        parser.add_argument(
            "--input-size",
            default=pdef.get("--input-size", 512),
            type=int,
        ...
        parser.add_argument(
            "--size-facts",
            default=pdef.get("--size-facts", [0.5, 0.75, 1]),
            nargs="+",
            type=float,
        ...
        parser.add_argument(
            "--label-smoothing-loss", action="store_true", help="For unsure labels"
        )
        parser.add_argument(
            "--focal-loss",
            action="store_true",
            help="In imbalanced ds, favor hard cases",
        )
        parser.add_argument(
            "--focal-loss-plus-ce-loss",
            action="store_true",
            help="Focal loss + cross entropy loss",
        )
        parser.add_argument(
            "--focal-loss-plus-dice-focal-loss",
            action="store_true",
            help="Focal loss + dice focal loss",
        )
        parser.add_argument("--ce-loss", action="store_true", help="cross entropy loss")
        parser.add_argument(
            "--weighted-loss",
            action="store_true",
            help="Uses weighted loss based on class distrib",
        )

        parser.add_argument(
            "--mixup",
            action="store_true",
            help="If MixUp should be used as regularization.",
        )
        return parser
    ...
    def get_exp_logdir(args, custom=""):
        ...
        if args.label_smoothing_loss:
            d += "_smooth-loss"
        elif args.focal_loss:
            d += "_focal-loss"
        elif args.focal_loss_plus_ce_loss:
            d += "_focal-plus-ce-loss"
        elif args.focal_loss_plus_dice_focal_loss:
            d += "_focal-plus-dice_focal-loss"
        elif args.ce_loss:
            d += "_ce-loss"
        if args.weighted_loss:
            d += "_weighted-loss"
        if args.progr_size:
            d += f'_progr-size{"_".join(map(str, args.size_facts))}'
        custom = f"{d}_{custom}"
    ...
    def prepare_training(args):
        ...
        if args.exp_logdir is None:
            args.exp_logdir = os.path.join(
                args.logdir, ImageTrainer.get_exp_logdir(args)
            )
    ...
    def create_cats_metrics(self, perf_fn, cat_id, cat, metrics_fn):
        """
        Generate metrics functions for the individual classes.
    ...
    def get_loss_fn(self, dls):
        ...
        """
        class_weights = (
            self.get_class_weights(dls.train_ds.items).to(dls.device)
            if self.args.weighted_loss
            else None
        )
        if self.args.label_smoothing_loss:
            loss_func = fmp.FixedLabelSmoothingCrossEntropyFlat(
                weight=class_weights, axis=self.loss_axis
            )
        elif self.args.focal_loss:
            loss_func = fmp.FixedFocalLossFlat(
                weight=class_weights, axis=self.loss_axis
            )
        elif self.args.focal_loss_plus_ce_loss:
            loss_func = closs.FocalLossPlusCElossFlat(
                weight=class_weights, axis=self.loss_axis
            )
        elif self.args.focal_loss_plus_dice_focal_loss:
            loss_func = closs.FocalLossPlusFocalDiceLoss(
                weight=class_weights, axis=self.loss_axis
            )
    ...
    def prepare_custom_metrics(self):
        """
        Create category specific metrics for all provided base metrics.

        :return dict, keys the function names, values the functions
        """
        metrics_fn = {}
        for perf_fn in self.args.metrics_base_fns:
            for cat_id, cat in zip(
                [None, *self.get_cats_idxs()], self.get_cats_with_all()
            ):
    ...
    def print_metrics_summary(self, metrics):
        ...
        for cid, cat in enumerate(self.get_cats_with_all()):
            ...
            cis = [
                metrics[f"{mn}{self.PERF_CI}"]
                if f"{mn}{self.PERF_CI}" in metrics
                else None
                for mn in mns
            ]
            cis = [
                ci if ci is None else f"({ci[0]*100:.0f}-{ci[1]*100:.0f})" for ci in cis
            ]
            s += (
                cat
                + ";"
                + ";".join([f"{metrics[mn]:.0%} {ci}" for mn, ci in zip(mns, cis)])
                + "\n"
            )
        print(s)
        return s
    ...
    def plot_custom_metrics(self, ax, agg_perf, show_val, title=None):
        ...
        bar_perf = {
            mn: cat_mres
            for p in self.args.metrics_base_fns
            for mn, cat_mres in agg_perf.items()
            if p in mn and self.PERF_CI not in mn
        }
        bar_cats = self.get_cats_with_all()
        cplot.grouped_barplot_with_err(
            ax, bar_perf, bar_cats, xlabel="Classes", show_val=show_val, title=title
        )
    ...
    def aggregate_test_performance(self, folds_res):
        ...
        for mns, agg_key in self.ordered_test_perfs_per_cats():
            ...
            if len(mns) > 0:
                agg[agg_key] = tuple(
                    np.stack(s) for s in zip(*[agg.pop(mn) for mn in mns if mn in agg])
                )
        return agg
    ...
    def ordered_test_perfs_per_cats(self):
        ...
        Each tuple is the list of metrics names (following cat order) with corresponding
        aggregated key
        """
        raise NotImplementedError
    ...
    def plot_save_path(self, test_path, run, show_val, custom=""):
        ...
        """
        return os.path.join(
            test_path, f'{run}{custom}{"_show_val" if show_val else ""}.jpg'
        )
    ...
    def tensorboard_cb(self, run_info):
        """
        Create tensorboard (TB) callback.

        :param run_info: str, run identifier for logs
        :return: TB callback
        """
        tbdir = common.maybe_create(self.args.exp_logdir, "tb_logs")
        return train_utils.ImageTBCb(
            tbdir, run_info, self.cust_metrics.keys(), self.ALL_CATS
        )
    ...
    def load_multiple_items_sets(self, set_locs, merged):
        """
        Load all sets of items.

        Merge them if specified.
        ...
        if set_locs is None:
            ...
            return (
                fv.L(map(fv.L, fv.L([t[1] for t in items_sets]).zip())).map(fv.L.concat)
                if merged
                else items_sets
            )
    ...
    def get_train_items(self, merged=True):
        ...
        return self.load_multiple_items_sets(sl, merged), self.load_multiple_items_sets(
            wl, merged
        )

    def get_full_img_cls(self, img_path):
        """
        Return class for full image.

        Should be reimplemented if stratification needed.
        Currently all full images will have same class.
        ...
        if self.stratify:
            ...
            return 1  # when stratified splits are not needed, consider all images have the same cls
    ...
    def get_full_img_dict(self, images, targets):
        """
        Match all patches with their full image.
        ...
        for img, tgt in zip(images, targets):
            full_images_dict[
                (self.get_patch_full_img(img), self.get_full_img_cls(img))
            ].append((img, tgt))
        return full_images_dict

    def split_data(self, items, items_cls):
        """
        Ensure that patches from the same image do not leak between train/val sets.
        ...
        for fold, tr, val in super().split_data(*fv.L(fi_dict.keys()).map_zip(fv.L)):
            ...
            if (
                not self.args.full_data
            ):  # if not True, then valid are patches from test set => no possible leaks
    ...
    def load_image_item(self, item, load_im_array=False):
        ...
        if type(item) is np.ndarray:
            return item  # image/mask is already loaded as np array
        elif self.args.encrypted:
            return crypto.decrypt_img(item, self.args.ckey)
        elif load_im_array:
            return cimg.load_img(item)
        else:
            return item
    ...
    def create_dls(self, tr, val, bs, size, **kwargs):
        ...
        datablock_args = {
            ...
            "item_tfms": fv.Resize(
                int(1.5 * self.args.input_size), method=fv.ResizeMethod.Squish
            ),
        ...
        if self.args.debug_dls:
            if train_utils.GPUManager.is_master_process():
                d.summary(self.args.data, bs=bs)
            sys.exit()
        # set path args so that learner objects use it
        return d.dataloaders(
            self.args.data,
            path=self.args.exp_logdir,
            bs=bs,
            seed=self.args.seed,
            **kwargs,
        )
    ...
    def maybe_progressive_resizing(self, tr, val, fold_suffix):
        """
        Create dataloaders with progressive resizing if requested.
        ...
        if self.args.progr_size:
            ...
            batch_sizes = [
                max(1, min(int(self.args.bs / f / f), len(tr[0])) // 2 * 2)
                for f in self.args.size_facts
            ]
        else:
        ...
        for it, (bs, size) in enumerate(zip(batch_sizes, input_sizes)):
            ...
            if self.args.progr_size:
                print(
                    f"Progressive resizing {it + 1}/{len(batch_sizes)}: running {run}"
                )
    ...
    def extract_run_params(self, run_info):
        """
        Extract run parameters from run info: #repeat, progr_size?, size, bs, fold.
    ...
    def load_learner_from_run_info(self, run_info, tr, val, mpath=None):
        """
        Create learner object from run information and items.

        If mpath set will load saved model weights.
