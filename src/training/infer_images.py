...
class ImageInference:
    ...
    def get_argparser(parser):
        """
        Add inference args to supplied arg parser.
        ...
        parser.add_argument(
            "--mpath",
            type=str,
            help="Model weights path (use if single model inference required)",
        )
        parser.add_argument(
            "--mdir",
            type=str,
            help="Model weights dir path (multiple models inference)",
        )
        parser.add_argument(
            "--impath", type=str, help="path of image to apply inference on"
        )
        parser.add_argument(
            "--imdir", type=str, help="path of dir with images to apply inference on"
        )
    ...
    def prepare_inference(args):
        """
        Check provided arguments validity.
        ...
        if args.mpath is not None:
            common.check_file_valid(args.mpath)
        else:
            if args.mdir is None and args.exp_logdir is not None:
                exp_logdir_mdir = os.path.join(args.exp_logdir, "models")
                if len(ImageInference.get_model_weights_files(exp_logdir_mdir)) > 0:
                    args.mdir = exp_logdir_mdir
            common.check_dir_valid(args.mdir)

        if args.impath is not None:
            common.check_file_valid(args.impath)
        elif args.imdir is not None:
            common.check_dir_valid(args.imdir)
        else:
            assert len(args.sl_tests) > 0, "No image to apply inference on was provided"
    ...
    def get_model_weights_files(dirpath):
        ...
        """
        return [
            m for m in common.list_files(dirpath, full_path=True) if m.endswith(".pth")
        ]
    ...
    def get_result_save_path(self, model_info):
        """
        Create dir where to save the inference results.
        ...
        """
        save_tag = (
            "_" + os.path.basename(self.args.mdir) if self.args.mdir is not None else ""
        )
        if self.args.ps is not None:
            save_tag += f"_ps{self.args.ps}px"
        return common.maybe_create(
            self.args.exp_logdir, f"preds{save_tag}_{self.args.exp_name}_{model_info}"
        )
    ...
    def inference_items(self):
        """
        Create list of inference items.
        ...
        if self.args.impath is not None:
            ...
            imgs = common.list_images(
                self.args.imdir, full_path=True, recursion=True, posix_path=True
            )
            return [(impath, self.maybe_get_labels(impath)) for impath in imgs]

    def prepare_learner_input(self, inference_item):
        """Convert inference item to learning input."""
        raise NotImplementedError

    def learner_inference(self, learn, save_dir):
        """
        Apply learner on inference items.
    ...
    def inference(self):
        ...
        """
        if self.args.mpath is not None:
            model_paths = [self.args.mpath]
        else:
            model_paths = ImageInference.get_model_weights_files(self.args.mdir)
        ...
        for mpath in model_paths:
            ...
            if self.args.inference_on_test_dirs:
                self.trainer.evaluate_on_test_sets(learn, run_info)
            else:
                self.learner_inference(learn, self.get_result_save_path(run_info))
