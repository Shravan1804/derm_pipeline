...
def tensors_mean_std(tensor_lst):
    """
    Take mean and std of tensor list.
...
def non_param_ci(tensor_lst, ci_p):
    """
    Compute the non-parametric confidence interval when predicted variable is not necessarily normally distributed.
    ...
    return torch.quantile(
        torch.stack(tensor_lst).float(), torch.tensor([low, high]), dim=0
    )


def split_model(model, splits):
    """
    Split model on requested top level children.

    Inspired from fastai 1. Used for freezing the model and adaptive learning rates
    ...
    assert (
        len(idxs) > 0
    ), f"Provided splits ({splits}) not found in top level children: {top_children}"
    if idxs[0] != 0:
        idxs = [0] + idxs
    if idxs[-1] != len(top_children):
        idxs.append(len(top_children))
    return [
        torch.nn.Sequential(*top_children[i:j]) for i, j in zip(idxs[:-1], idxs[1:])
    ]
...
def show_tensors_in_memory(gpu_only=True):
    ...
    def pretty_size(size):
        return " × ".join(map(str, size)) if isinstance(size, torch.Size) else size
    ...
    def print_tensor_like_obj_details(obj):
        if not gpu_only or obj.is_cuda:
            ...
            details += (
                f'{" grad" if obj.requires_grad else ""} {pretty_size(obj.data.size())}'
            )
            print(details)
...
class GPUManager:
    ...
    def in_distributed_mode():
        """
        Check if running in distributed mode. True when script was launched with distributed_launch.py.
    ...
    def is_master_process():
        """
        Check if process is the master process (gpu rank is 0).

        :return: bool, is master process
        """
        return (
            GPUManager.distributed_rank() == 0
    ...
    def init_distributed_process():
        """Start distributed mode. Should be called before beginning of training."""
        if GPUManager.in_distributed_mode():
    ...
    def running_context(learn, device_ids=None):
        """
        Create appropriate gpu running context (parallel or distributed).
        ...
        return (
            learn.distrib_ctx()
            if GPUManager.in_distributed_mode()
            else learn.parallel_ctx(device_ids)
        )
...
class ImageTBCb(fc.TensorBoardBaseCallback):
    ...
    def can_run(self):
        """Check if callback can run. Must be master process."""
        return (
    ...
    def after_batch(self):
        ...
        self.writer.add_scalar(
            f"{self.run_info}_Loss/train_loss",
            getattr(self, "smooth_loss", -1),
            self.train_iter,
        )
        for i, h in enumerate(self.opt.hypers):
            for k, v in h.items():
                self.writer.add_scalar(
                    f"{self.run_info}_Opt_hyper/{k}_{i}", v, self.train_iter
                )

    def after_epoch(self):
        """Write epochs metrics."""
        if not self.run:
            return
        ...
        for n, v in zip(self.recorder.metric_names[2:-1], self.recorder.log[2:-1]):
            if n in self.grouped_metrics:
                ...
                if self.all_cats in cat_code:
                    reduced[n] = v
                else:
                    grouped[perf][cat_code] = v
            else:
                log_group = "Loss" if "loss" in n else "Metrics"
                self.writer.add_scalar(
                    f"{self.run_info}_{log_group}/{n}", v, self.train_iter
                )
        for perf, v in grouped.items():
            self.writer.add_scalars(
                f"{self.run_info}_Metrics/{perf}", v, self.train_iter
            )
        self.writer.add_scalars(
            f"{self.run_info}_Metrics/ALL", reduced, self.train_iter
        )
