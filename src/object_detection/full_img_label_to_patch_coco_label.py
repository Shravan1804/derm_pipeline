...
def main(args):
    """Perform full image to patch dataset conversion.
    ...
    for img, pms in tqdm(img_pms.items()):
        for pm in pms:
            new_image_patches.append(
                {
                    "width": ps,
                    "license": 0,
                    "file_name": pm["patch_path"],
                    "id": patch_id,
                    "height": ps,
                }
            )
            ...
            for anno in f_annos[img]:
                ...
                a_poly = Polygon(
                    [
                        anno["segmentation"][0][n : n + 2]
                        for n in range(0, len(anno["segmentation"][0]), 2)
                    ]
                )
                ...
                if patch_poly.intersects(a_poly):
                    ...
                    inters = (
                        [inters] if type(inters) is Polygon else inters
                    )  # Multipolygon case
                    for inter in inters:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1024,
        type=int,
        help="patch size to convert labels to",
    )
