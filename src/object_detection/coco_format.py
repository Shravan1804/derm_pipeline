...
def get_categories(cat_names, cat_ids):
    """
    Prepare list of category dicts.
    ...
    """
    return [
        {"id": i, "name": cat_names[i - 1], "supercategory": cat_names[i - 1]}
        for i in sorted(cat_ids)
    ]
...
def convert_obj_mask_to_rle(mask):
    """
    Convert binary array mask to rle dict and converts counts information to utf-8.
...
def convert_obj_mask_to_poly(mask):
    """
    Convert array binary mask to polygon coordinates.
    ...
    for poly in mask_utils.convert_binary_mask_to_polys(mask):
        ...
        assert len(segmentation) % 2 == 0 and 3 <= len(segmentation) // 2, (
            "Wrong polygon points: %s" % segmentation
        )
...
def convert_torch_masks_to_rle(masks):
    """
    Convert torch category masks to rle format.
...
def get_annos_from_objs_mask(
    img_id, start_ann_id, obj_cats, obj_cats_masks, scores=False, to_poly=False
):
    """
    Extract all object annotations from array mask.
    ...
    for oc, (nb_objs, oc_mask) in zip(obj_cats, obj_cats_masks):
        for obj_id in range(
            1, nb_objs
        ):  # first object 0 is the background, last obj id is nb_objs-1
...
def get_img_annotations(img_id, start_ann_id, targets):
    ...
    """
    assert start_ann_id > 0, (
        "ann_id MUST start at 1 since pycocotools.cocoeval uses detId to track matches"
        "and checks with > 0"
    )
    annos = []
    categories = set()
    ...
    for i in range(len(targets["labels"])):
        annos.append(
            get_obj_anno(
                img_id,
                ann_id,
                int(targets["labels"][i]),
                [float(b) for b in targets["boxes"][i]],
                int(targets["areas"][i]),
                targets["masks"][i],
            )
        )
...
def visualize_dataset(img_dir, anno_json, dest):
    ...
    for img in tqdm(imgs):
        ...
        coco.showAnns(anns)
        cplot.plt_save_fig(
            os.path.join(dest, str(img["id"]).zfill(4) + "_" + img["file_name"]),
            dpi=150,
        )
...
def save_sample_coco_data(coco_json_path, sample_size=100, save_path=None, seed=42):
    ...
    with open(coco_json_path, "r") as f:
        ...
        for img in sample["images"]:
            sample["annotations"].extend(
                [a for a in coco_json["annotations"] if a["image_id"] == img["id"]]
            )
        save_path = (
            coco_json_path.replace(".json", "_sample.json")
...
if __name__ == "__main__":
    ...
    visualize_dataset(
        path + "/images", path + "/full_img_segm_test_set.json", path + "/results"
    )
