...
class ImageObjectDetectionTrainer(ImageTrainer):
    ...
    @staticmethod
    def get_argparser(
        desc="Fastai image obj detec trainer arguments", pdef=dict(), phelp=dict()
    ):
        """
        Create object detection argparser.
        ...
        """
        parser = super(
            ImageObjectDetectionTrainer, ImageObjectDetectionTrainer
        ...
        parser.add_argument(
            "--with-segm",
            action="store_true",
            help="Metrics will be also computed on segmentation",
        )
        return parser
    ...
    def prepare_training(args):
        ...
        if args.with_segm:
            assert (
                args.model == "mask_rcnn"
            ), f"Used --with-segm but requested model is {args.model}"
    ...
    def __init__(self, args, stratify=False, full_img_sep=CROP_SEP, **kwargs):
        ...
        self.metrics_types = [ia.COCOMetricType.bbox] + (
            [ia.COCOMetricType.mask] if args.with_segm else []
        )
        ...
        self.coco_parser_cls = (
            obj_utils.COCOMaskParserEncrypted
            if self.args.encrypted
            else ia.parsers.COCOMaskParser
        )
    ...
    def load_coco_anno_file(self, anno_file):
        ...
        coco_parser = self.coco_parser_cls(
            annotations_filepath=anno_path, img_dir=img_dir
        )
        idmap = ia.IDMap()
        records = fv.L(
            coco_parser.parse(
                data_splitter=ia.SingleSplitSplitter(), idmap=idmap, show_pbar=False
            )[0]
        )
    ...
    def load_items(self, anno_file):
        """
        Load training items from annotation file.
    ...
    def get_cat_metric_name(self, perf_fn, cat, iou, mtype):
        """
        Get category specific metric name for object detection metrics.
    ...
    def get_cat_segm_metric_name(self, perf_fn, cat, score):
        """
        Get category specific metric name for segmentation metrics.
    ...
    def create_cats_metrics(self, perf_fn, cat_id, cat, metrics_fn):
        """
        Generate metrics functions for the individual categories and different iou and metric types.
        ...
        for mtype in self.metrics_types:
            for iou in self.args.ious:
                ...
                cls = custom_coco_eval_metric(
                    cls_name, cat_id=cat_id, iou=iou, metric=perf_fn, metric_type=mtype
                )
    ...
    def ordered_test_perfs_per_cats(self):
        ...
        for perf_fn in self.args.metrics_base_fns:
            for mtype in self.metrics_types:
                for iou in self.args.ious:
                    mns = [
                        self.get_cat_metric_name(perf_fn, cat, iou, mtype)
        ...
        if self.args.with_segm:
            for perf_fn in self.args.metrics_base_fns:
                mns = [
                    self.get_cat_segm_metric_name(perf_fn, cat, None)
                    for cat in self.get_segm_cats_with_all()
                ]
                ordered.append((mns, f"{self.SEGM_PERF}{perf_fn}"))
                for score in self.scores_steps:
                    mns = [
                        self.get_cat_segm_metric_name(perf_fn, cat, score)
                        for cat in self.get_segm_cats_with_all()
                    ]
    ...
    def compute_conf_mat(self, targs, preds):
        ...
        Valid only if OD with segmentation.
    ...
    def compute_test_predictions(self, learn, test_name, test_items_with_cls):
        """
        Apply OD model to test items.

        Also prepare segmentation predictions if relevant.
        :param learn: learner
        ...
        with GPUManager.running_context(learn, self.args.gpu_ids):
            ...
            interp.od_targs, interp.od_preds = arch.predict_dl(
                model=learn.model, infer_dl=test_dl
            )
        ...
        if self.args.with_segm:
            ...
            im_no_gt = [
                (i, imd["file_name"])
                for i, imd in parser._imageid2info.items()
                if i not in idmap.name2id
            ]
            with GPUManager.running_context(learn, self.args.gpu_ids):
                od_preds_no_gt, empty_gt = [], None
                for batch in tqdm(common.batch_list(im_no_gt, bs=learn.dls.bs)):
                    ims = [
                        self.load_image_item(os.path.join(imdir, im_name))
                        for _, im_name in batch
                    ]
                    batch_dl, samples = arch.build_infer_batch(
                        ia.Dataset.from_images(ims, vtfms)
                    )
                    if empty_gt is None:
                        empty_gt = np.zeros((samples[0]["height"], samples[0]["width"]))
                    od_preds_no_gt.append(
                        arch.predict(model=learn.model, batch=batch_dl)
                    )
                od_preds_no_gt = {
                    i: p
                    for (i, _), p in zip(
                        im_no_gt, [x for b in od_preds_no_gt for x in b]
                    )
                }
            ...
            for imid, imdict in parser._imageid2info.items():
                if imid in idmap.name2id:
                    ...
                    gtm = mask_utils.merge_obj_masks(
                        interp.od_targs[r]["masks"].data, interp.od_targs[r]["labels"]
                    )
                    gt_masks.append(gtm)
                ...
                for s, p in pred_masks.items():
                    ...
                    p.append(
                        mask_utils.merge_obj_masks(om[oidx], ol[oidx])
                        if oidx.any()
                        else empty_gt.copy()
                    )
            interp.segm_targs = torch.stack([torch.tensor(gtm) for gtm in gt_masks])
            interp.segm_decoded = {
                s: torch.stack([torch.tensor(p) for p in sp])
                for s, sp in pred_masks.items()
            }
    ...
    def evaluate_on_test_sets(self, learn, run):
        ...
        for test_name, test_items_with_cls in self.get_test_items(merged=False):
            ...
            interp = self.compute_metrics(
                self.compute_test_predictions(learn, test_name, test_items_with_cls)
            )
    ...
    def compute_metrics(self, interp):
        ...
        for mn, mfn in self.cust_metrics.items():
            ...
            interp.metrics[mn] = torch.tensor(
                next(iter(mfn.finalize().values()))
            ).float()
        if self.args.with_segm:
            segm_perf = partial(
                segm_utils.cls_perf, cats=self.args.cats, bg=None, axis=None
            )
            for s, preds in interp.segm_decoded.items():
                interp.metrics[f"{self.SEGM_PERF}cm_score{s}"] = self.compute_conf_mat(
                    interp.segm_targs, preds
                )
            for fname, pfn in [
                (fname, getattr(train_utils, fname))
                for fname in self.args.metrics_base_fns
            ]:
                for cid, cat in zip(
                    [None, 0, *self.get_cats_idxs()], self.get_segm_cats_with_all()
                ):
                    all_scores_res = []
                    for s in self.scores_steps:
                        mn = self.get_cat_segm_metric_name(fname, cat, s)
                        interp.metrics[mn] = segm_perf(
                            pfn, interp.segm_decoded[s], interp.segm_targs, cid
                        )
                        all_scores_res.append(interp.metrics[mn])
                    interp.metrics[
                        self.get_cat_segm_metric_name(fname, cat, None)
                    ] = torch.stack(all_scores_res)
        return interp

    def plot_precision_recall(self, ax, pre_with_err, rec_with_err, show_val):
        """
        Plot Precision-Recall plot for all detection confidence score thresholds.
        ...
        svals = (
            [[f"{(r, p)}" for r, p in zip(rr, pp)] for rr, pp in zip(rec, pre)]
            if show_val
            else None
        )
        cplot.plot_lines_with_err(
            ax, rec, pre, self.get_segm_cats_with_all(), pstd, rstd, svals
        )
        score_steps = np.repeat(
            self.scores_steps[None,], len(self.get_segm_cats_with_all()), 0
        )
        best_scores = [
            sorted(zip(*x))[-1][-1] for x in zip(pre + rec, pre, rec, score_steps)
        ]
    ...
    def plot_test_performance(self, test_path, run, agg_perf):
        ...
        od_agg_perf = {
            k: v for k, v in agg_perf.items() if not k.startswith(self.SEGM_PERF)
        }
        fig, axs = cplot.new_fig_with_axs(
            1, len(self.metrics_types), self.args.test_figsize, sharey=True
        )
        for ax, mtype in zip(
            [axs] if len(self.metrics_types) < 2 else axs, self.metrics_types
        ):
            mtype_agg_perf = {
                k: v for k, v in od_agg_perf.items() if k.endswith(f"_{mtype.name}")
            }
            self.plot_custom_metrics(
                ax, mtype_agg_perf, show_val, title=f"OD {mtype.name} metrics"
            )
        ...
        if self.args.with_segm:
            segm_agg_perf = {
                k: v for k, v in agg_perf.items() if k.startswith(self.SEGM_PERF)
            }
            fig, axs = cplot.new_fig_with_axs(1, 2, self.args.test_figsize)
            pre_with_err, rec_with_err = tuple(
                segm_agg_perf[f"{self.SEGM_PERF}{f}"] for f in ("precision", "recall")
            )
            best_score = self.plot_precision_recall(
                axs[0], pre_with_err, rec_with_err, show_val
            )
            cplot.plot_confusion_matrix(
                axs[1],
                agg_perf[f"{self.SEGM_PERF}cm_score{best_score}"],
                self.args.cats,
            )
            fig.tight_layout(pad=0.2)
            save_path = self.plot_save_path(
                test_path, run, show_val, custom="_segm_perf"
            )
            plt.savefig(save_path, dpi=400)
    ...
    def get_tfms(self):
        ...
        train_tfms = ia.tfms.A.Adapter(
            [
                *ia.tfms.A.aug_tfms(
                    size=self.args.input_size, presize=int(1.5 * self.args.input_size)
                ),
                ia.tfms.A.Normalize(),
            ]
        )
        valid_tfms = ia.tfms.A.Adapter(
            [*ia.tfms.A.resize_and_pad(self.args.input_size), ia.tfms.A.Normalize()]
        )
    ...
    def create_dls(self, tr, val, bs, size):
        ...
        return ImageObjectDetectionTrainer.convert_ia_dls_to_fastai_dls(
            [train_dl, valid_dl]
        )
    ...
    def create_learner(self, dls):
        ...
        model = arch.model(
            num_classes=len(ia.ClassMap(self.args.cats[1:])), **arch_params
        )
        return arch.fastai.learner(
            dls=dls, model=model, metrics=self.cust_metrics.values()
        )
...
if __name__ == "__main__":
    ...
    parser = ImageObjectDetectionTrainer.get_argparser(
        desc="Fastai image object detection", pdef=defaults
    )
