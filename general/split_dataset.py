...
import common
import common_img as cimg


def knapSack(W, wt, val):
    """Solves knapSack problem, source https://www.geeksforgeeks.org/0-1-knapsack-problem-dp-10/
...
def knapSack_with_items(W, wt, val, items):
    """Solves knapSack problem, source https://www.geeksforgeeks.org/printing-items-01-knapsack/
    :param W: total capacity to fill
    :param wt: weights of items
    :param val: values of items
...
def test_knapSack():
    """Simple example of the knapSack problem"""
    items = ['a','b','c']
    val = [ 21, 22, 29 ]
    wt = [ 21, 22, 29 ]
...
def split_and_save(category, dirpath, test_prop, dest_root, pidx_fn, seed):
    """Splits directory and save images in respective dir
    ...
    for setname, impaths in zip(('train', 'test'), (train_impaths, test_impaths)):
        ...
        for impath in impaths:
            ...
            cimg.save_img(cimg.load_img(impath), os.path.join(dest_dir, os.path.basename(impath)))




def split_classif_dataset(root_dir, test_prop, pidx_fn, seed):
    """Splits typical classification dataset
    ...
    fn = partial(split_and_save, test_prop=test_prop, dest_root=dest_root, pidx_fn=pidx_fn, seed=seed)
    p_umap(fn, classes, [os.path.join(root_dir, c) for c in classes])




def split_segm_dataset(root_dir, test_prop, pidx_fn, seed, imdir='images', mdir='masks'):
    """Splits typical segmentation dataset
    ...
    """
    dest_root = common.maybe_create(root_dir.rstrip("/") + "_splitted")
    split_and_save(imdir, os.path.join(root_dir, imdir), test_prop, dest_root, pidx_fn, seed)
    for d in ['train', 'test']:
        ...
        for imname in tqdm(common.list_images(os.path.join(dest_root, d, imdir))):
            ...
            cimg.save_img(cimg.load_img(mpath), os.path.join(dest_mdir, os.path.basename(mpath)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pidx-fn', type=str, default="usz", help="Function to extract patient idx")
