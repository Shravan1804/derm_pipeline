...
"""common.py: File regrouping useful methods for plotting"""
...
def zero_error_bars(vals):
    """Creates (low, high) error bars for provided values arrays
...
def init_and_clip_err(vals, err, bounds=(0, 1)):
    ...
    if err.shape == (2, *vals.shape):
        ...
        return np.abs(np.vstack([vals - err, vals + err]).clip(*bounds) - np.vstack([vals, vals]))


def get_error_display_params():
    """Helper function to get usual error display parameters
...
def show_graph_values(ax, values, pos_x, pos_y=None, yerr=None, fontsize=6):
    """Writes values on provided axis
    :param ax: axis
...
def plot_lines_with_err(ax, xs, ys, labels, yerrs=None, xerrs=None, show_vals=None, legend_loc="lower center",
                        err_bounds=(0, 1)):
...
def grouped_barplot_with_err(ax, stats, groupLabels, xlabel=None, ylabel=None, title=None, barwidth=.35, show_val=False,
    ...
    """Creates barplot with several groups
    :param ax: axis
    ...
    for offset, (key, (vals, err)), c in zip(offsets, stats.items(), [cmap(i) for i in range(nbars_per_group)]):
        ...
        if show_val:
            show_graph_values(ax, [f'{v:.2f}' for v in vals], positions + offset - barwidth/2, pos_y=vals, yerr=err[1])
...
def plot_confusion_matrix(ax, cm_std, labels, title=None, title_loc=None):
    """Plots confusion matrix with std values below. Code inspired from fastai library.
    :param ax: axis
...
def new_fig_with_axs(nrows, ncols, base_fig_width, base_fig_height=None, **kwargs):
    """Creates new fig according to provided params
...
def plt_fig_as_np_array(fig):
    """Converts matplotlib figure in image array
...
def prepare_img_axs(h_w_ratio, nrows, ncols, figsize_fact=8, no_axis=True, flatten=True, title=None):
    """Creates figure with size fitting provided h/w ratio
