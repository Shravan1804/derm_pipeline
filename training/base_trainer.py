...
"""base_trainer.py: Base trainer class. Should be extendable to any training tasks (image, text, etc)."""
...
class FastaiTrainer:
    ...
    def get_argparser(desc="Fastai trainer arguments", pdef=dict(), phelp=dict()):
        """Creates base trainer argparser
        ...
        parser.add_argument('--sl-train', type=str, default=['train'], nargs='+', help="strong labels (sl) dirs")
        parser.add_argument('--sl-tests', type=str, default=['test'], nargs='+', help="sl test dirs")
    ...
    def prepare_custom_metrics(self):
        """Creates task specific custom metrics, should be extended"""
        raise NotImplementedError

    def get_test_items(self):
        """Get test items, should be extended"""
    ...
    def load_learner_from_run_info(self, run_info, tr, val, mpath=None):
        """Load learner object, should be extended
    ...
    def create_dls(self):
        """Create dataloader, should be extended"""
        raise NotImplementedError

    def train_procedure(self, tr, val, fold_suffix, run_prefix="", learn=None):
        """Train learner. Creates learner if no learner provided. Should be extended.
        :param tr: tuple of train items and labels
        :param val: tuple of valid items and labels
    ...
    def correct_wl(self, wl_items, preds):
        """Corrects weak labeled items with model predictions. Should be extended.
    ...
    def get_sorting_run_key(self, run_info):
        """Gets key from run info string allowing to aggregate runs results. Should be extended.
        :param run_info: str, run info string
        """
        raise NotImplementedError

    def plot_test_performance(self, test_path, agg):
        """Plot test performance. Should be extended.
    ...
    def get_train_cbs(self, run):
        """Custom training callbacks: tensorboard, ReduceLROnPlateau, early stopping
    ...
    def prepare_learner(self, learn):
        """Final learner preparation step after creation. Currently adds half precision, and brings model to gpu.
    ...
    def basic_train(self, run, learn, dls=None, save_model=True):
        """Training the learner, called by train_procedure method.
    ...
    def evaluate_and_correct_wl(self, learn, wl_items, run):
        """Evaluates and corrects weak labeled items, clears GPU memory (model and dls)
        :param learn: learner object
        :param wl_items: tuple of weak labeled items and labels
        :param run: str, run information string
    ...
    def generate_tests_reports(self):
        """Aggregates fold predictions, plots performance, save predictions results"""
    ...
    def train_model(self):
        """Training loop with cyclic weak labels refinement"""
